#!/usr/bin/env bash
# -*- coding: utf-8 -*-
# wsl-setup.sh — Автонастройка WSL2 для DevOps-среды
set -euo pipefail
IFS=$'\n\t'

### 0. Проверка среды и конвертация CRLF → LF ###
if ! grep -qi microsoft /proc/version; then
  echo "❌ Ошибка: скрипт предназначен для WSL2!" >&2
  exit 1
fi
# Если вы скачивали прямо через curl, возможно есть \r
[ -f "$0" ] && sed -i 's/\r$//' "$0"
...
curl -fsI https://github.com >/dev/null || die "Нет сети — проверьте подключение"
...
sudo apt install -y \
  ...
  docker.io docker-compose jq fzf ripgrep bat direnv gnupg2 ca-certificates pv dos2unix unzip
...
install_tool(){
  ...
  info "Установка $name $ver"
  tmp=$(mktemp -d)
  curl -fsSL "$url" -o "$tmp/$name.tar.gz" || die "Не удалось скачать $name"
  tar -xzf "$tmp/$name.tar.gz" -C "$tmp" || die "Ошибка распаковки $name"
...
info "Настройка Zsh и Powerlevel10k"
...
for t in "${!ASDF_TOOLS[@]}"; do
  repo=${ASDF_TOOLS[$t]% *}
  ver=${ASDF_TOOLS[$t]#* }
  info "asdf: установим $t $ver"
  asdf plugin-remove "$t" &>/dev/null || true
...
tmp=$(mktemp -d)
curl -fsSL "$URL" -o "$tmp/$ZIP" || die "Не удалось скачать Terraform"
unzip -q "$tmp/$ZIP" -d "$tmp" || die "Ошибка распаковки Terraform"
